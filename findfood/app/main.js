...
app.controller('MainController', function ($scope, $state, $http,loginServices) {
    ...
    $scope.logout = function () {
        ...
        angular.element(document.querySelector('#error')).removeClass('displayblock');
        angular.element(document.querySelector('#error')).addClass('displaynone');
        $state.go('home');
    }
    ...
    $scope.changeReview = function (review, distance) {
        ...
        .then(function (data) {
            if (data != 'error')
                ...
                angular.element(document.querySelector('#error')).addClass('displayblock');
                $scope.food = null;
                $scope.place = null;
            }
        },
...
app.controller('FavouritesController', function ($scope, $http, loginServices, $state) {
    ...
    $http.post('/getReviewedItems', { username: loginServices.getUsername() })
       .success(function (res) {
           ...
           $scope.numReviewed = result.length
       }).error(function (data) { console.log(data); });
    ...
    $scope.isFavouritesEmpty = function () {
        if (angular.isUndefined($scope.userFavourites))
            return true;
        else
            return false;
    }

    $scope.isFavouritesNotEmpty = function () {
        if (!angular.isUndefined($scope.userFavourites))
            return true;
        else
            return false;
...
app.controller('ReviewController', function ($scope, $state, $stateParams, $http, loginServices, $window) {
    angular.element(document.querySelector('#displayreview')).addClass('displaynone');
    $scope.business = angular.fromJson($stateParams.business);
    $scope.username = loginServices.getUsername();
    ...
    $scope.submit = function (review) {
        console.log(review);
        if (angular.isUndefined(review)) {
            $window.alert("plase write review");
        }
        else {
            $http.post('/saveReview', { username: loginServices.getUsername(), business: $scope.business, review: review })
...
app.controller('ViewReviewController', function ($scope, $http, loginServices) {
    ...
    $http.post('/getReviewedPlaces', { username: loginServices.getUsername() }).success(function (data) {
        $scope.objects = data.obj;
        $scope.reviewCount = $scope.objects.length;
    }).error(function (error) { console.log(error) });

    $scope.reviewed = function () {
        if ($scope.reviewCount != 0)
            return true;
        else
            return false;
    }
        ...
        return {
            search: function (food, place, sort) {
                ...
                $http.post('/search', { food: food, place: place, sort: sort }).success(function (data) {
                    if (angular.isUndefined(data.businesses))
